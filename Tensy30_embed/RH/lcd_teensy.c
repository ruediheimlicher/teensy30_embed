...
lcd_load_byte(uint8_t out_byte)
{
   ...
   digitalWriteFast(LCD_CLOCK_PIN, LOW);
   int i;
   for(i=0; i<8; i++)
   {
      /* loop through bits */
      ...
      if (out_byte & 0x80)	// Maske 1000 0000
      {
         ...
         digitalWriteFast(LCD_RSDS_PIN, HIGH);
      }
      else
      {
         /* this bit is low */
         //LCD_PORT &= ~_BV(LCD_RSDS_PIN);
         digitalWriteFast(LCD_RSDS_PIN, LOW);
      }
      out_byte = out_byte << 1;
      
      /* pulse the the shift register clock */
      //LCD_PORT |= _BV(LCD_CLOCK_PIN);	
      digitalWriteFast(LCD_CLOCK_PIN, HIGH);
...
lcd_strobe_E(void)
{
   ...
   digitalWriteFast(LCD_ENABLE_PIN, HIGH);
        _delay_us(450); 
	//	lcddelay_ms(100);
    //    LCD_PORT &= ~_BV(LCD_ENABLE_PIN);
   digitalWriteFast(LCD_ENABLE_PIN, LOW);
}
